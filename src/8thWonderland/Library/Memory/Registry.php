...
class Registry {
    ...
    {
        if (!is_array($options)) {
            throw new Exception('Invalid options provider : This must be an array !');
        }
        $options = array_change_key_case($options, CASE_LOWER);
        
        if (!empty($options['session_name']) && is_string($options['session_name'])) {
            session_name($options['session_name']);
        }
        if (!empty($options['session_cache_expire']) && is_int($options['session_cache_expire'])) {
            session_cache_expire($options['session_cache_expire']);
        }
    }
